$(document).ready(function () {
    ...
    function assignedward_data() {
        $.ajax({
            ...
            success: function (data) {
                ...
                function assignwardTable(wardObj) {
                    (function rec(d) {
                        $.each(d, function (k, v) {
                    ...
                    var wardListDatatable = $("#assignedWardListDT").DataTable({
                        "data": wardObj,
                        responsive: true,
                        paging: true,
                        searching: true,
                            ...
                            },
                            {
                                ...
                                "render": function (data, type, row) {
                                    return (
                                        ...
                                        <a href="javascript:void(0)">WP ID: #' + data + '<br> [Bed No: ' + row.bed_no + '] <span style="text-align:left"></span></a>\
                                    </h2>'
                                    );
    ...
    $('#assignedWardListDT').on('click', '.ActionBtn', function () {
        ...
        $(".WardAdmitID").text(thisData.id);
        $(".WardAdmitDate").text(moment(new Date(thisData.assigned_date)).format("DD-MM-YYYY"));
        $(".WardName").text(thisData.ward_mame);
        $(".WardBedNo").text(thisData.bed_no);
        $(".WardAdmitReason").text(thisData.reason);
        ...
        if (parseInt(user_status) == 0 ) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        function displayAmount(){
                            ...
                            $('#room_amount').val(data['ward_assigned_amount'].room_amount)
                        ...
                        $('#scan_amount').val(data['ward_assigned_amount'].scan_amount)
                        $('.Scan_fees').text(data['ward_assigned_amount'].scan_amount)

                            var payment_recived_by_lab = (parseInt(data['ward_assigned_amount'].lab_amount)  - parseInt(data['ward_assigned_amount'].balance_lab_amount)) 
                        $('#paid_lab_amountText').text(payment_recived_by_lab)

                            var payment_recived_by_xray = (parseInt(data['ward_assigned_amount'].xray_amount)  - parseInt(data['ward_assigned_amount'].balance_xray_amount)) 
                            $('#paid_xray_amountText').text(payment_recived_by_xray)
    
                            var payment_recived_by_scan = (parseInt(data['ward_assigned_amount'].scan_amount)  - parseInt(data['ward_assigned_amount'].balance_scan_amount)) 
                            $('#paid_scan_amountText').text(payment_recived_by_scan)
    
                            total_amount = parseInt(data['ward_assigned_amount'].main_total);
                            // total_amount = parseInt($('#xray_amount').val() + $('#lab_amount').val() + $('#scan_amount').val());
                            main_balance = parseInt(data['ward_assigned_amount'].main_balance) 

                        $("#overallTotal").val(total_amount)
                        $("#overall_totaltext").text(total_amount);
                        $("#Ward_Total_Amount").text(total_amount);
                ...
                $(".feesAmount").on("keyup keydown", function () {
                    ...
                    $(".feesAmount").each(function () {
                        if (!isNaN($(this).val()) && ($(this).val()).length != 0) {
                            ...
                            $("#Ward_Total_Amount").text(total_amount + sum);
                            ...
                            if (parseInt($("#discount_amount").val()) > 0) {
                                $("#Ward_Total_Amount").text(total_amount + sum - parseInt($("#discount_amount").val()))
                    ...
                    if (sum > 0) {
                        ...
                        if (ExistingBalance + main_balance >= sum ) {
                            ...
                            $("#overallBalance").val((ExistingBalance + main_balance) - parseInt(sum) - parseInt($("#paymentReceived").val()))
                                $("#Ward_Total_Amount").text(totalAmount - sum)
                        ...
                        $("#Ward_Total_Amount").text(totalAmount)
            ...
            $(".discountValue").on("keyup", function () {
                ...
                if (sum > 0) {
                    ...
                    $("#discount_amount").val(parseInt(sum));
                    $("#Ward_Total_Amount").text(parseInt($('#overallTotal').val()) - sum);
                    ...
                    $("#Ward_Total_Amount").text($('#overallTotal').val());
    ...
    function pendingAmountData() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var DataObj = data['ward_assigned_amount'];

                var WardPayment = DataObj.ward_payment_data;
                
                $('#physiotherapy_charges').val(DataObj.ward_payment_data.physiotherapy_charges);
                $('#nursing_charge').val(DataObj.ward_payment_data.nursing_charge);
                $('#establishment_charges').val(DataObj.ward_payment_data.establishment_charges);
                $('#iv_fluid_charges').val(DataObj.ward_payment_data.iv_fluid_charges);
                $('#icu_charges').val(DataObj.ward_payment_data.icu_charges);
                $('#physiotherapy_charges').val(DataObj.ward_payment_data.physiotherapy_charges);
                $('#surgery_charges').val(DataObj.ward_payment_data.surgery_charges);
                $('#dressing_charges').val(DataObj.ward_payment_data.dressing_charges);
                $('#miscellaneous_charges').val(DataObj.ward_payment_data.miscellaneous_charges);
                $('#consultant_charges').val(DataObj.ward_payment_data.consultant_charges);
                
                
                $('#ward_amount').val(DataObj.ward_amount)
                $('.Ward_fees').text(DataObj.ward_amount)

                $('#ward_charge').val(DataObj.ward_payment_data['ward_charge'])
                ...
                $('#overallTotal').val(DataObj.ward_payment_data['overall_total'])

                $('#overall_totaltext').text(DataObj.ward_payment_data['overall_total']);
                $('#Ward_Total_Amount').text(DataObj.ward_payment_data['overall_total']);
                $('#totalAmount_after_discountText').text(DataObj.ward_payment_data['overall_total'])
                // $('#discount').val(DataObj.ward_payment_data['total_after_discount'])
                

                $('#existing_discount').val(DataObj.ward_payment_data['discount'])
                $("#existingDiscountText").text(DataObj.ward_payment_data['discount']);

                $("#balanceAmountText").text(DataObj.ward_payment_data['balance']);
                $("#balanceValue").val(DataObj.ward_payment_data['balance']);
                $("#existingpaidAmountText").text(DataObj.ward_payment_data['already_paid']);
                $("#existing_balance").val(parseInt(DataObj.ward_payment_data['balance']));
                $("#ward_PatientPaymentId").val(parseInt(DataObj.ward_payment_data['id']));

                $("#doctor_fees").val(DataObj.ward_payment_data['doctor_fees'])
                $("#injection").val(DataObj.ward_payment_data['injection'])
                $("#service").val(DataObj.ward_payment_data['service'])
                $("#others").val(DataObj.ward_payment_data['others'])
                ...
                $.each(DataObj.charge_details, function(feetext, feevalue) {
                    if(parseFloat(feevalue) > 0){
                        ...
                        $(".Ward_Invoice_Section").append(AppendText);
                    }
                ...
                $.each(DataObj.payment_data, function(text, value) {
                    if(parseFloat(value) > 0){
                        ...
                        $(".Ward_Payment_Section").append(AppendText);
                    }
                ...
                if(DataObj.ward_payment_data['discount'] >0){
                    $(".Invoice_AmountSection").append('<p class="mb-0">Discount <span class="Invoice_Discount pull-right">₹ ' + DataObj.ward_payment_data['discount'] +'</span></p>');
                }

                $(".Invoice_AmountTotal").append('<h4>Total Amount <span class="Invoice_Total">₹ ' +  (parseInt(DataObj.main_total) - parseInt(DataObj.ward_payment_data['discount'])) +'</span></h4>');

                // $(".Invoice_AmountTotal").append('<h4>Total Amount <span class="Invoice_Total">₹ ' + DataObj.ward_payment_data['total_after_discount']+'</span></h4>');
                
                if(DataObj.ward_payment_data['already_paid'] > 0) {
                    $(".Invoice_Paid").text('₹ ' + DataObj.ward_payment_data['already_paid']);
    ...
    $('#discharge_Btn').click(function () {
        ...
        Swal.fire({
            ...
            if (result.value == true) {
                $.ajax({
                    type : 'GET',
                    url: $('#discharge_patient').data('url') + "?ward_assignid=" + $('#Ward_assignid').val()+"&bed_id=" + $("#bed_id").val(),
                    success : function(){
                        location.reload()
                    },
                    error: function (exception) {
    ...
    $("#DoctorPrescriptionForm").submit(function (e) {
        ...
        var PatientObj = {
            Ward_assignid: $("#Ward_assignid").val(),
