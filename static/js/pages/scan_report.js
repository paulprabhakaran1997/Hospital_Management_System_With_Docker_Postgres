$(document).ready(function()
{
    ...
    function getReportData(from_date , to_date) {
        var GetScanReportUrl = $("#GetScanReport").data('url')
        $.ajax({
            type: "GET",
            url: GetScanReportUrl + "?from_date=" + from_date+"&to_date=" + to_date,
            success : function(data){
                console.log("Data = " , data.scantestfromallcatogory)
                PatientData_Scan_DT(data.scantestfromallcatogory)
                console.log("Transaction Data = ", data.scanpaymenttransaction)
                ScanPaymentTransactionDT(data.scanpaymenttransaction)
            },
    ...
    function ScanPaymentTransactionDT(dataObj) {
        (function rec(d) {
            $.each(d, function (k, v) {
                if (typeof v === 'object') return rec(v)
                if (isNaN(v) && typeof v === 'number') d[k] = '---';
        ...
        if ($.fn.dataTable.isDataTable("#scanPaymentTransactionListDT")) {
            $("#scanPaymentTransactionListDT").DataTable().destroy();
        }

        var patientListDatatable = $("#scanPaymentTransactionListDT").DataTable({
            data: dataObj,
            responsive: true,
            paging: true,
            "scrollY": false,
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        ...
                        return (
                            '<h2 class="table-avatar">\
                                <a href="javascript:void(0)"> <span style="text-align:left"> ( '+type+' ID : ' + data + ' )</span> <span style="text-align:left"> ( Scan Tested ID : ' + row.scan_tested_id + ' )</span> </a>\
                            </h2>'
                        )
                    }
                },
